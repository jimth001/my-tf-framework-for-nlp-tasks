...
from DeepComponents.AttentionMask import padding_attention_mask, left_to_right_attention_mask
...
def block(x, scope, *, past, hparams, src_seq_mask=None):
    with tf.variable_scope(scope):
        nx = x.shape[-1].value
        a, present = attn(norm(x, 'ln_1'), 'attn', nx, past=past, hparams=hparams, src_seq_mask=src_seq_mask)
        x = x + a
...
def positions_for(tokens, past_length):
    """calculate position id for tokens based on past_length"""
