...
class BPETool():
    def __init__(self,codes_path):
        fcodes=open(codes_path, encoding='utf-8')
        self.bpe = BPE(fcodes, -1, '@@', None, None)
        fcodes.close()

    def apply_bpe(self,sentence):
        return self.bpe.segment(sentence).replace("p@@ u@@ nc", "<PUNC>").replace("p@@ un@@ c", "<PUNC> ").strip()
...
if __name__ == '__main__':
    ...
    for line in args.input:
        args.output.write(bpe.segment(line).replace("p@@ u@@ nc","<PUNC>").replace("p@@ un@@ c","<PUNC> ").strip())
        args.output.write('\n')
